...
public class Grafo {
    ...
    //cria o vertice de entrada do grafo...acredito que para efeitos de pesquisa e organização...
    ...
    public void setaLicacao() {
        ...
        for (int externo = 0; externo < bolinhas.size(); externo++) {
            // sequencial vem da flag 'fLAG', e significa q o proximo vertice se liga ao atual
            //se o vertice B1 tiver fLAG, quer dizer q o B2 se liga a ele
            //primeiro confiro se getfLAG n é null, depois vejo se é verdadeiro, pois talvez tenha chance de ser null
            if (bolinhas.get(externo).get(0).getfLAG() != null && bolinhas.get(externo).get(0).getfLAG() && externo + 1 < bolinhas.size()) {
    ...
    public void imprimeSaidas(){
        System.out.println("Tamanho: "+bolinhas.size());
        for(int externo = 0; externo < bolinhas.size(); externo++){
