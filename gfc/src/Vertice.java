...
public class Vertice {
    //contem coleção de comandos. Um deve ser o lider, o outro a saida, o resto 'bloco'
    private String id;
    private List<Comando> partes = new ArrayList<>();//posso deixar a 1° posição como lider, e a ultima como saida? Melhor ter especifico
    private Comando lider;
    private Comando saida;
    private Boolean saiu = false;
    private Boolean proxEDestinoGOTO = false;
    private Boolean fLAG = true;//durante a execução do grafo, se um vertice tiver fLAG, quer dizer q a proxima linha é uma
    ...
    private void setSaida(Comando linha){
        if(linha.getContemGOTO()){
            ...
            if(linha.getContemIF()){//Se contem if, ele vai seguir para o proximo tmb, além do goto
                this.fLAG = true;
                //durante a execução do grafo, se um vertice tiver fLAG, quer dizer q a proxima linha é uma
                //ligação dele.
            }
        }
    }
    ...
    public void setLider(Comando candidato) {
        ...
        partes.add(candidato); //o lider tmb faz parte do bloco maior de comandos...esse é msm um bom conceito?
